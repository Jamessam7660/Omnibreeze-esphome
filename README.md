...
- **220Ω resistor** (1/4W, through-hole — see Resistor Selection Guide below)
...
Summary:
  ...
  ZO        → ESP32 GPIO5 (direct) AND ESP32 GPIO6 (through 220Ω resistor)
  QG (5V)   → DO NOT CONNECT
```

**Key detail:** GPIO6 connects to **both** RF (direct wire) and ZO (through a series resistor). This allows the Tuya component to send heartbeat/init frames to RF (keeping the MCU happy) while also sending weakened commands to ZO that don't overpower the MCU's own transmissions on that line.

### Resistor Selection Guide

The resistor value is the trickiest part of this project. It must be low enough for commands to reach the MCU, but high enough to prevent the ESP32's TX from overpowering the MCU's status transmissions on ZO. **The optimal value varies between setups** — it depends on your specific ESP32 board's GPIO drive strength, wire length, solder joint quality, and possibly even the fan MCU's batch. There's no single "correct" value that works for everyone.

**Start with 220Ω** (red-red-brown-gold) and work from there:

| Resistor | Notes |
|----------|-------|
| **220Ω** | ✅ Start here — worked most reliably in testing |
| 330Ω | Try if 220Ω causes echo/init issues |
| 470Ω | Another option if lower values cause problems |
| 1KΩ | Worked on one setup but failed on others — try if nothing else works |
| 100Ω | Usually too low — echo corrupts status reads |
| 47Ω | Essentially a short, won't help |

**If commands don't work:** try a lower value. Work down: 220Ω → 100Ω.

**If status is garbled or Tuya init keeps failing:** try a higher value. Work up: 220Ω → 330Ω → 470Ω → 1KΩ.

**Quick test method:** before soldering, twist the resistor legs around bare wire ends and test with loose connections. Once you find the value that works, then solder it permanently.

**Why the variability?** Honestly, it's not fully understood. The half-duplex bus arbitration between the ESP32 TX and the fan MCU TX is analog in nature — small differences in signal strength, impedance, and timing can shift the sweet spot. If you find a value that works, please report it in a GitHub issue along with your ESP32 board model so we can build a better picture.
...
1. Check that GPIO6 is connected to **both** RF (direct) and ZO (through resistor). Without the RF connection, the MCU won't accept commands.
2. Try a **lower resistor value**. If using 1KΩ, try 470Ω, then 330Ω, then 220Ω. The 220Ω value has been the most reliable across multiple fan units.
3. Verify the resistor is in the **GPIO6→ZO path only** — the GPIO6→RF path must be direct with no resistor.
4. Check solder joints — a cold joint can look connected but have high resistance. Test continuity with a multimeter.

### Status updates not appearing
Check that GPIO5 is connected to ZO. Check that the resistor is in the GPIO6→ZO path (not in the GPIO6→RF path). If status was working before adding the resistor, try a higher value (470Ω → 1KΩ).
...
6. A series resistor on the TX→ZO path was the final breakthrough for half-duplex — **220Ω** proved most reliable, though the optimal value may vary between units (tested range: 100Ω–1KΩ)
...
- The resistor value is sensitive to the ESP32 board's GPIO drive strength — 1KΩ worked on some setups but was too high on others; 220Ω was the most consistently reliable value
- The fan's 5V rail (QG) cannot supply enough current to power an ESP32-S3 (~500mA peak) — external USB power is required
...
- Confirming which resistor value works best on your setup (report your ESP32 board model and resistor value)
- Confirming the Auto mode (DP 2, value 3) behavior
- Identifying DP 24 (bitmask) function
- Finding a power supply solution to run the ESP32 from the fan's internal 5V rail
